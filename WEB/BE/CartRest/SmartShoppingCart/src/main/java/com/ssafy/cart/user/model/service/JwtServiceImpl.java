...
public class JwtServiceImpl implements JwtService {
	...
	private static final String SALT = "sscssafySecretSeCrEtKeYgIfJwVqOPfgJmXnapi";
	private static final int ACCESS_TOKEN_EXPIRE_MINUTES = 10; // 분단위
	private static final int REFRESH_TOKEN_EXPIRE_MINUTES = 2; // 주단위
	
//	private Key secretKey = Keys.secretKeyFor(SignatureAlgorithm.HS256);
//	private Key secretKey = Algorithm.HMAC256(this.generateKey());
	// private Key secretKey = Keys.hmacShaKeyFor(this.generateKey());
...
//	
	...
	public <T> String createAccessToken(String key, T data) {
		log.info("JwtServiceImpl createAccessToken - 호출");
//		return create(key, data, "access-token", 1000 * 60 * ACCESS_TOKEN_EXPIRE_MINUTES);
	...
	public <T> String createRefreshToken(String key, T data) {
		log.info("JwtServiceImpl createRefreshToken - 호출");
//		return create(key, data, "refresh-token", 1000 * 60 * 60 * 24 * 7 * REFRESH_TOKEN_EXPIRE_MINUTES);
		return create(key, data, "refresh-token", 1000 * 60 * 2 * REFRESH_TOKEN_EXPIRE_MINUTES);
	}
	...
	public <T> String create(String key, T data, String subject, long expire) {
		log.info("JwtServiceImpl create - 호출");
//		log.info("key : {}", key);
...
//			parseClaimsJws : 파싱하여 원본 jws 만들기
			Jws<Claims> claims = Jwts.parserBuilder().setSigningKey(SALT.getBytes("UTF-8")).build().parseClaimsJws(jwt);
